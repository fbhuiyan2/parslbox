...
from parslbox.apps.app_registry import (
    get_app_config, is_app_registered, get_app_instance, get_registered_apps
)
from parslbox.configs.loader import get_system_config, load_config
...
class ParslBox:
    ...
    def update_job(
        ...
        if add_deps is not None or rm_deps is not None:
            ...
            existing_parents = database.parse_existing_parents(current_job.get('parents'))
            ...
            if all_parent_ids:
                invalid_ids = database.validate_parent_job_ids(self.db_path, all_parent_ids)
                if invalid_ids:
                    raise ValidationError(
                        f"Parent job IDs do not exist: {invalid_ids}"
                    )
            ...
            # Process removals
            if rm_deps:
                updated_parents, _ = database.remove_dependencies(updated_parents, rm_deps)
    ...
    def filter_jobs(
        ...
        """
        jobs = self.list_jobs(status=status, app=app, tag=tag, path=path, in_file=in_file)
    ...
    def qsub(
        ...
        if not filesystems:
            pbs_template = '\n'.join(
                line for line in pbs_template.split('\n')
        ...
        try:
            ...
            raise ValidationError(
                "qsub command not found. Make sure PBS is available."
            )
